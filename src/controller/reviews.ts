...
import { getBusinessById, businessRatingById } from "../services/business";
import { Request, Response } from "express";

export async function getReviewsBusinessByIdController(
  req: Request,
  res: Response,
) {
  try {
    ...
    if (!reviews) {
      return res
        .status(400)
        .json({ error: "No se pudo obtener la informacion" });
    }
...
export async function getAllReviewsBusinessController(
  req: Request,
  res: Response,
) {
  try {
...
export async function upsertReviewBusinessController(
  req: Request,
  res: Response,
) {
  try {
...
export async function upsertRatingBusinessController(
  req: Request,
  res: Response,
) {
  try {
    ...
    return res
      .status(200)
      .json({ message: "Calificacion agregada exitosamente" });
  } catch (err: any) {
    return res.status(400).json({ error: err.message || err });
  }
}
export async function deleteReviewBusinessController(
  req: Request,
  res: Response,
) {
  try {
